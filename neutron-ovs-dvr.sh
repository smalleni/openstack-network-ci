...
function overcloud_deploy() {
    if [ $OSP_VERSION -ge 12 ]; then
        if [ $NEUTRON_BACKEND == OVS ]; then        
           ...
           time openstack overcloud deploy --templates -e /usr/share/openstack-tripleo-heat-templates/environments/network-isolation.yaml -e templates/network-environment.yaml -e templates/deploy.yaml -e /home/stack/docker_registry.yaml -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-opendaylight.yaml --ntp-server clock.redhat.com > overcloud_deploy.log 2>&1
...
git clone https://a171d26e38f06eb38c9f13bf5852a3a83ca8139c@github.com/redhat-performance/browbeat-config.git
...
openstack subnet create --allocation-pool start=172.21.0.110,end=172.21.0.200 --gateway=172.21.0.1 --no-dhcp --network public --subnet-range 172.21.0.0/24 public_subnet
