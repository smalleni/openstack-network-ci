...
git clone https://github.com/redhat-performance/ops-tools.git
...
function overcloud_deploy() {
    if [ $OSP_VERSION -ge 12 ]; then
        if [ $NEUTRON_BACKEND == OVS ]; then        
            ...
            time openstack overcloud deploy --templates -e /usr/share/openstack-tripleo-heat-templates/environments/network-isolation.yaml -e templates/network-environment.yaml -e templates/deploy.yaml -e /home/stack/docker_registry.yaml -e /usr/share/openstack-tripleo-heat-templates/environments/neutron-ovs-dvr.yaml -e templates/neutron-policy.yaml --ntp-server clock.redhat.com > overcloud_deploy.log 2>&1
        elif [ $NEUTRON_BACKEND == ODL ]; then
           openstack overcloud container image prepare --namespace=${INSECURE_REGISTRY}/${NAMESPACE} --push-destination 192.168.24.1:8787 --prefix=openstack --tag=${TAG} -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-opendaylight.yaml --images-file /home/stack/overcloud_containers.yaml --output-env-file /home/stack/docker_registry.yaml
           openstack overcloud container image upload --config-file /home/stack/overcloud_containers.yaml
           time openstack overcloud deploy --templates -e /usr/share/openstack-tripleo-heat-templates/environments/network-isolation.yaml -e templates/network-environment.yaml -e templates/deploy.yaml -e /home/stack/docker_registry.yaml -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-opendaylight.yaml -e templates/opendaylight-transactions.yaml -e templates/neutron-policy.yaml --ntp-server clock.redhat.com > overcloud_deploy.log 2>&1
        elif [ $NEUTRON_BACKEND == OVN ]; then
           openstack overcloud container image prepare --namespace=${INSECURE_REGISTRY}/${NAMESPACE} --push-destination 192.168.24.1:8787 --prefix=openstack --tag=${TAG} -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-ovn-ha.yaml --images-file /home/stack/overcloud_containers.yaml --output-env-file /home/stack/docker_registry.yaml
           openstack overcloud container image upload --config-file /home/stack/overcloud_containers.yaml
           time openstack overcloud deploy --templates -e /usr/share/openstack-tripleo-heat-templates/environments/network-isolation.yaml -e templates/network-environment.yaml -e templates/deploy.yaml -e /home/stack/docker_registry.yaml -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-ovn-ha.yaml -e templates/neutron-policy.yaml --ntp-server clock.redhat.com > overcloud_deploy.log 2>&1
        fi
    else
        if [ $NEUTRON_BACKEND == OVS ]; then        
            sudo sed -i  "/^\s*OS::TripleO::Compute::Ports::ExternalPort:/c \  OS::TripleO::Compute::Ports::ExternalPort: /usr/share/openstack-tripleo-heat-templates/network/ports/external.yaml" /usr/share/openstack-tripleo-heat-templates/environments/neutron-ovs-dvr.yaml
           ...
           popd
           time openstack overcloud deploy --templates -e /usr/share/openstack-tripleo-heat-templates/environments/network-isolation.yaml -e templates/network-environment.yaml -e templates/deploy.yaml -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-opendaylight.yaml -e templates/neutron-policy.yaml --ntp-server clock.redhat.com > overcloud_deploy.log 2>&1
        elif [ $NEUTRON_BACKEND == OVN ]; then           
           time openstack overcloud deploy --templates -e /usr/share/openstack-tripleo-heat-templates/environments/network-isolation.yaml -e templates/network-environment.yaml -e templates/deploy.yaml -e /usr/share/openstack-tripleo-heat-templates/environments/services-docker/neutron-ovn-ha.yaml -e templates/neutron-policy.yaml --ntp-server clock.redhat.com > overcloud_deploy.log 2>&1
        fi      
    fi
}
overcloud_deploy
...
sed -i "/^\s*ext_net_id:/c \        ext_net_id: ${EXTERNAL_NET}" browbeat/browbeat-config.yaml
EOSSH
